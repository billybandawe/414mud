...
E     := gameentities
M     := main
FILES := $(M)/FourOneFourMud $(M)/Connection $(M)/Commandset $(M)/Orcish $(E)/Stuff $(E)/Character $(E)/Player $(E)/NPC $(E)/Object $(E)/Container $(E)/Money $(E)/Room # gamelogic/Command gamelogic/package-info
SDIR  := mud
BDIR  := bin
BACK  := backup
ICON  := go.sh #icon.ico
